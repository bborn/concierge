module Concierge
  module Spike
    ...
    # The CSM is no longer *the* engine; it is agent #1 (+:csm+).
    #
    #   c.agent :billing do
    ...
    class Agent
      ...
      # Slot 1b — the model this agent runs on. Falls back to the global
      # +default_model+ so an agent only names one when it needs a cheaper or
      # stronger one than the house default.
      dsl_value :model
      ...
      # Slot 3 — least-privilege tool scope, per function.
      ...
      # Scope::SHARED one. A second, settable identifier would be a way to
      # accidentally alias two agents onto one memory pool — the exact
      # cross-function contamination §10.3 exists to prevent.
      def memory_namespace
        slug.to_s
      end

      def to_s
        "#<Concierge::Spike::Agent #{slug}>"
      end
    end
  end
end
